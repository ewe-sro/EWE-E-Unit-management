...
current_month = datetime.now().strftime("%m%y")
# Get the current month and year in mm/yy format
current_month_message = datetime.now().strftime("%m/%y")
    
# Email configuration
email_sender = config["EmailSettings"]["EmailSender"]
email_receiver = config["EmailSettings"]["EmailReceiver"]
email_subject = f"EWE - Charging data - {current_month}"
email_message = f"V příloze vám zasíláme souhrn dat nabíjecí stanice za období {current_month_message}"
